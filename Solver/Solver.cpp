...
bool Solver::optimize(Solution &sln, float &object, ID workerId) {
   ...
   /* for (int i = 0; i != pcenter.size(); i++) {
    ...
    iter = 1;
    clock_t mid_tim;
    while ((mid_tim - start_time)*1.0 / CLOCKS_PER_SEC < 100)//搜索条件
    {
        flag = 1;
        Pair = { -1 };
...
int Solver::find_pair(Solution &sln, Scinfo &ScInfo, pair &Pair,ID nodeNum, ID centerNum) {//确定交换对<nodei，centerj>
   ...
   // cout << "haha" << endl;
    if ((tabu_pair.delt < no_tabu_pair.delt) && tabu_pair.delt < ScInfo.Sc)// && (tabu_pair.delt > 0.01)) //解禁条件：禁忌解优于当前查找的非禁忌解中最好的且优于历史最优解
    {
